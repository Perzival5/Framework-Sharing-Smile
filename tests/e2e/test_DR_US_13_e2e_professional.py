...
def test_DR_TC_331_Professional(get_url):
    ...
    response_get = request_function(StaticDataVerbs.get.value, get_url, f"{StaticDataModules.professionals.value}{response_post.json()['id']}",
                                header_type=StaticDataHeaders.header_professional.value)
    
    request_patch =build_user_payload()
    response_patch = request_function(StaticDataVerbs.patch.value, get_url, f"{StaticDataModules.professionals.value}{response_get.json()['id']}",
                                header_type=StaticDataHeaders.header_professional.value, payload=request_patch, files=get_file_edit())
    
    response_delete = request_function(StaticDataVerbs.delete.value, get_url, f"{StaticDataModules.professionals.value}{response_patch.json()['id']}",
